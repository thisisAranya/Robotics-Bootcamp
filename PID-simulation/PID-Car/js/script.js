...
// Sliders, Outputs, Number Inputs, and Data Displays
const sliders = {
  kp: document.getElementById("kpSlider"),
  ki: document.getElementById("kiSlider"),
  kd: document.getElementById("kdSlider"),
...
const inputs = {
  kp: document.getElementById("kpInput"),
  ki: document.getElementById("kiInput"),
  kd: document.getElementById("kdInput")
};
const outputs = {
  kp: document.getElementById("kpVal"),
  ki: document.getElementById("kiVal"),
...
// PID parameters and internal states
let pid = { 
  kp: 0.4, 
  ki: 0.015, 
  kd: 0.5, 
...
function setupEventListeners() {
  ...
  for (const key in sliders) {
    sliders[key].oninput = () => {
      ...
      if (key === 'speed') car.velocity = parseFloat(val);
      else if (key === 'scale') pathParams.scale = parseFloat(val);
      else updatePIDValuesFromUI();
    };
  }

  // Sync inputs → sliders & outputs
...
function setPreset(kp, ki, kd) {
  ...
  outputs.ki.textContent = ki.toFixed(3);
  outputs.kd.textContent = kd.toFixed(3);
  if (inputs.kp) inputs.kp.value = kp;
  if (inputs.ki) inputs.ki.value = ki;
  if (inputs.kd) inputs.kd.value = kd;
  updatePIDValuesFromUI();
}
...
function gameLoop(timestamp) {
  ...
  const clampedDt = Math.max(0.002, Math.min(dt, 0.05));
...
function getLaneCenterY(x) {
  ...
  switch(pathParams.type) {
    ...
    case 'circle':
      const radius = amplitude;
      const circumference = 2 * Math.PI * radius;
      const angle = (x * 2 * Math.PI) / circumference;
      return centerY + radius * Math.sin(angle);
...
function drawGraph() {
  ...
  graphCtx.font = "12px Inter";
  let legendX = 10;
  graphCtx.fillStyle = colors.blue;
  graphCtx.fillText('Error', legendX, 15);
  legendX += 45;
  graphCtx.fillStyle = colors.green;
  graphCtx.fillText('P', legendX, 15);
  legendX += 20;
  graphCtx.fillStyle = colors.orange;
  graphCtx.fillText('I', legendX, 15);
  legendX += 20;
  graphCtx.fillStyle = colors.red;
  graphCtx.fillText('D', legendX, 15);
}
